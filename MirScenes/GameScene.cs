...
namespace Client.MirScenes
{
    ...
    public sealed class MapControl : MirControl
    {
        ...
        public static Dictionary<int, DX_Texture> MiniMaps = new Dictionary<int, DX_Texture>();
        ...
        public long OutputDelay;
        public DX_Texture MiniMapTexture;
        ...
        public void CreateMiniMap()
        {
            ...
            //var texture = DX_Vortice.DX_NewTextureNoneManaged(Width, Height);
            //var stream = DX_Vortice.DX_TextureLockRectangle(texture);
            using (Bitmap image = new Bitmap(Width, Height, PixelFormat.Format32bppArgb))
            {
                ...
                nint datapoint = 0;
                texture = CreateTextureFromBytes(GetBitmapPixelData(image), (uint)Width, (uint)Height, ref datapoint);
            }
            //DX_Vortice.DX_TextureUnlockRectangle(texture);
        ...
        protected override void CreateTexture()
        {
            ...
            if (ControlTexture == null)
            {
                ...
                ControlTexture = DX_Vortice.DX_NewTextureRenderTargetDefault(Size.Width, Size.Height);
                TextureSize = Size;
            }

            var oldSurface = DX_Vortice.CurrentSurface;
            var surface = DX_Vortice.DX_GetSurfaceLevel0(ControlTexture);
            DXManager.SetSurface(surface);
            DX_Vortice.DX_DeviceClearTarget00(BackColour);

            if (FloorValid)
            {
                DXManager.Draw(DX_Vortice.FloorTexture, new Rectangle(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), Vector3.Zero, Color.White);
            }
        ...
        {
            //if (DX_Vortice.FloorTexture == null || DX_Vortice.FloorTexture.Disposed)
            if (DX_Vortice.FloorTexture == null)
            {
                DX_Vortice.FloorTexture = DX_Vortice.DX_NewTextureRenderTargetDefault(Settings.ScreenWidth, Settings.ScreenHeight);
                DX_Vortice.FloorSurface = DX_Vortice.DX_GetSurfaceLevel0(DX_Vortice.FloorTexture);
            }

            var oldSurface = DX_Vortice.CurrentSurface;
            DXManager.SetSurface(DX_Vortice.FloorSurface);
            DX_Vortice.DX_DeviceClearTarget00(Color.Empty); //Color.Black
        ...
        {
            if (DX_Vortice.Lights == null || DX_Vortice.Lights.Count == 0) return;
            
            //if (DX_Vortice.LightTexture == null || DX_Vortice.LightTexture.Disposed)
            if (DX_Vortice.LightTexture == null)
            {
                DX_Vortice.LightTexture = DX_Vortice.DX_NewTextureRenderTargetDefault(Settings.ScreenWidth, Settings.ScreenHeight);
                DX_Vortice.LightSurface = DX_Vortice.DX_GetSurfaceLevel0(DX_Vortice.LightTexture);
            }

            var oldSurface = DX_Vortice.CurrentSurface;
            DXManager.SetSurface(DX_Vortice.LightSurface);
            ...
            DX_Vortice.DX_DeviceClearTarget00(darkness);
            ...
            DX_Vortice.DX_DeviceSetRenderStateSourceAlphaOne();
            ...
            for (int i = 0; i < Objects.Count; i++)
            {
                ...
                if (ob.Light > 0 && (!ob.Dead || ob == MapObject.User || ob.Race == ObjectType.Spell))
                {
                    ...
                    if (lightRange >= DX_Vortice.Lights.Count)
                        lightRange = DX_Vortice.Lights.Count - 1;
                    ...
                    //if (DX_Vortice.Lights[lightRange] != null && !DX_Vortice.Lights[lightRange].Disposed)
                    if (DX_Vortice.Lights[lightRange] != null)
                    {
                        p.Offset(-(DXManager.LightSizes[lightRange].X / 2) - (CellWidth / 2), -(DXManager.LightSizes[lightRange].Y / 2) - (CellHeight / 2) -5);
                        DXManager.Draw(DX_Vortice.Lights[lightRange], null, new Vector3((float)p.X, (float)p.Y, 0.0F), lightColour);
                    }
                }
                ...
                for (int e = 0; e < ob.Effects.Count; e++)
                {
                    ...
                    //if (DX_Vortice.Lights[light] != null && !DX_Vortice.Lights[light].Disposed)
                    if (DX_Vortice.Lights[light] != null)
                    {
                        p.Offset(-(DXManager.LightSizes[light].X / 2) - (CellWidth / 2), -(DXManager.LightSizes[light].Y / 2) - (CellHeight / 2) - 5);
                        DXManager.Draw(DX_Vortice.Lights[light], null, new Vector3((float)p.X, (float)p.Y, 0.0F), lightColour);
                    }
            ...
            for (int y = MapObject.User.Movement.Y - ViewRangeY - 24; y <= MapObject.User.Movement.Y + ViewRangeY + 24; y++)
            {
                ...
                for (int x = MapObject.User.Movement.X - ViewRangeX - 24; x < MapObject.User.Movement.X + ViewRangeX + 24; x++)
                {
                    ...
                    if (light >= DX_Vortice.Lights.Count)
                        light = DX_Vortice.Lights.Count - 1;
                    
                    //if (DX_Vortice.Lights[light] != null && !DX_Vortice.Lights[light].Disposed)
                    if (DX_Vortice.Lights[light] != null)
                    {
                        p.Offset(-(DXManager.LightSizes[light].X / 2) - (CellWidth / 2) + 10, -(DXManager.LightSizes[light].Y / 2) - (CellHeight / 2) - 5);
                        DXManager.Draw(DX_Vortice.Lights[light], null, new Vector3((float)p.X, (float)p.Y, 0.0F), lightIntensity);
                    }
                }
            }
            #endregion
            ...
            DX_Vortice.DX_DeviceSetRenderStateZeroSourceColor();

            DXManager.Draw(DX_Vortice.LightTexture, new Rectangle(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), Vector3.Zero, Color.White);

            DX_Vortice.DX_SpriteEnd();
            DX_Vortice.DX_SpriteBeginAlphaBlend();
        }
