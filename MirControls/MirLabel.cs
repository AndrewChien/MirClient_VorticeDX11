    ...
    public class MirLabel : MirControl
    {
        ...
        protected override unsafe void CreateTexture()
        {
            ...
            if (ControlTexture == null)
            {
                ...
                //ControlTexture = DX_NewTextureNoneManaged(Size.Width, Size.Height);
                TextureSize = Size;
            }

            //var stream = DX_TextureLockRectangle(ControlTexture);
            using (Bitmap image = new Bitmap(Size.Width, Size.Height, PixelFormat.Format32bppArgb))
            {
                using (Graphics graphics = Graphics.FromImage(image))
                {
                    ...
                    nint datapoint = 0;
                    ControlTexture = CreateTextureFromBytes(GetBitmapPixelData(image), (uint)Size.Width, (uint)Size.Height, ref datapoint);
                }
            }

            //DX_TextureUnlockRectangle(ControlTexture);
            DX_Flush();
            TextureValid = true;
        }
